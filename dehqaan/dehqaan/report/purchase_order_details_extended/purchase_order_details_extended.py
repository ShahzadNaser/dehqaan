...
def execute(filters=None):
    ...
    totals = frappe._dict({})
    totals.update({
                "indent": 0.0, "doctype":'', "name":'Grand Total', "parent_department":'',
    ...
    blank_totals.update({
                "indent": 0.0, "doctype":'', "name":'', "parent_department":'',
                        "department": '', "workflow_state": '', "project": '',
    ...
    for department in departments:
        ...
        department_totals.update({
                "indent":  0.0, "doctype":'', "name":'Total', "parent_department":'',
                        "department": '', "workflow_state": '', "project": '',
                        "project_name":'', "supplier_name": '', "grand_total":0.0,
                        "outstanding": 0.0,'transaction_date':'','paid':0.0,
                        "advance_paid":0.0,"invoiced":0.0
        ...
        for ss in order_details:
            ...
            if department.department == ss.cost_center:
                temp = frappe._dict({})
                temp.update({
			"indent": 1.0, "doctype":'Purchase Order', "name": ss.name, "parent_department": ss.cost_center,
		...
		if False and department.department == departments[len(departments)-1].department:
			
			temp = frappe._dict({})
				temp.update({
				"indent": 1.0, "doctype":'Expense Claim', "name": "EXP-22-0101", "parent_department": department.department,
				"department": '', "workflow_state": "", "project": filters.project,
				"project_name": "", "supplier_name": "", "grand_total": "",
							"outstanding": "",'transaction_date':"",'paid':"",'advance_paid':"",
							"invoiced":""
					})
			data.append(temp)
			frappe.errprint("=============================================")
			frappe.errprint(department.department == departments[len(departments)-1].department)
			frappe.errprint("=============================================")
		frappe.errprint("{} {}".format(department.department , departments[len(departments)-1].department))
